...
async function showRecentFast(message, fast, fastIsInProgress, currentTimestamp) {
    ...
    fastEmbed = new Discord.MessageEmbed()
        ...
        .setDescription(fastDataToString);
...
async function getUserEdit(userOriginalMessageObject, fastFields, fieldToEdit) {
    ...
    do {
        messageIndex++;
        collectedEdit = await fn.messageDataCollectFirst(userOriginalMessageObject, fastEditMessagePrompt, "Fast: Fast Edit", "00FF00", 600000);
        if (collectedEdit === "stop") {
            return false;
        }
        if (messageIndex === 1 || reset === true) {
            if (collectedEdit == "1") {
                let endEditConfirmation = await getEditEndConfirmation(userOriginalMessageObject, field, userEdit);
...
async function confirmPostOverwrite(userOriginalMessageObject, overwriteReplaceWith, overwriteTitle = "Default") {
...
async function getFastPostEmbed(userOriginalMessageObject, fastData) {
    ...
    do {
        ...
        if (attachment === null) {
            collectedObject = await fn.messageDataCollectFirstObject(userOriginalMessageObject, fastPostMessagePrompt, "Fast: Post Creation", "#00FF00", 1800000,
                true, 3000);
        }
        else {
            collectedObject = await fn.messageDataCollectFirstObject(userOriginalMessageObject, fastPostMessagePrompt, "Fast: Post Creation", "#00FF00", 1800000,
        ...
        if (collectedMessage == "2") {
            // Overwrite any previously collected data: Confirm first, if confirmed exit and post, otherwise continue loop
            let confirmOverwrite = await confirmPostOverwrite(userOriginalMessageObject, "**full fast post (including mood and reflection)**", "Full Fast Post")
            if (confirmOverwrite === true) {
    ...
    while (true)
    if (attachment === null) {
        fastPost = await fn.getMessageEmbed(fastPost, "Fast Post", "#00FF00");
    }
    else {
        fastPost = await fn.getMessageImageEmbed(attachment, fastPost, "Fast Post", "#00FF00");
    }
    return fastPost;
}
...
async function postFast(bot, userOriginalMessageObject, fastPost, endTimestamp) {
...
module.exports.run = async (bot, message, args) => {
    ...
    switch (fastCommand) {
        ...
        case "end":
            ...
            fastEndUsageMessage = fn.getMessageEmbed(fastEndUsageMessage, `Fast: Fast End Help`, "#00FF00");
            ...
            if (fastIsInProgress == 0) {
                ...
                if (quickEnd == "❌") {
                    ...
                    moodValue = await fn.userSelectFromList(message, "", 5, moodValuePrompt, "Fast: Mood Assessment", "#008000");
                    if (moodValue === false) {
                        return;
                    }
                    var reflectionText;
                    ...
                    let reset = false;
                    do {
                        let userReflection = await fn.messageDataCollectFirst(message, reflectionTextPrompt, "Fast: Reflection", "#00FF00", 900000);
                ...
                fastCollectionDocument.collection.updateOne({
                    ...
                    .then(async () => {
                        ...
                        if (confirmPostFast === false) {
                            ...
                            let fastPost = await getFastPostEmbed(message, fastData);
                            if (fastPost === false) {
                                return;
                            }
                            await postFast(bot, message, fastPost, endTimestamp);
                        }
                    })
        ...
        case "see":
            ...
            fastSeeUsageMessage = fn.getMessageEmbed(fastSeeUsageMessage, `Fast: Fast See Help`, "#00FF00");
            ...
            // it will be handled properly later based on the values passed in!
                ...
                if (args[1].toLowerCase() == "recent") {
                    ...
                    if (pastNumberOfEntriesIndex <= 0) {
                        fn.sendErrorMessageAndUsage(message, fastSeeHelpMessage, "**FAST DOES NOT EXIST**...", true);
                        return;
                    }
                ...
                if (pastFunctionality) {
                    ...
                    for (i = 0; i < pastNumberOfEntriesIndex; i++) {
                        ...
                        if (fastView[i] === undefined) {
                            ...
                            fn.sendErrorMessageAndUsage(message, fastSeeHelpMessage, `**FAST ${i + 1}**+ ONWARDS DOES NOT EXIST...`, false);
                            break;
                        }
                        ...
                        if (args[3] !== undefined) {
                            ...
                            if (!isNaN(args[3])) {
                                ...
                                for (i = 0; i < pastNumberOfEntriesIndex; i++) {
                                    if (fastView[i] === undefined) {
                                        pastNumberOfEntriesIndex = i;
                                        fn.sendErrorMessageAndUsage(message, fastSeeHelpMessage, `**FAST ${i + entriesToSkip + 1}**+ ONWARDS DOES NOT EXIST...`, false);
                                        break;
                                    }
                                    let fastData = fastCursorToDataArray(fastView[i]);
                                    fastDataToString = fastDataToString + `__**Fast ${i + entriesToSkip + 1}:**__\n` + fastDataArrayToString(fastData) + "\n";
                                }
                                fastEmbed = fn.getMessageEmbed(fastDataToString, `Fast: See ${pastNumberOfEntriesIndex} Fasts Past ${entriesToSkip}`, "#00FF00");
                                message.channel.send(fastEmbed);
                                return;
                            }
                            else {
                ...
                if (fastView === undefined) {
                    fn.sendErrorMessageAndUsage(message, fastSeeHelpMessage, "**FAST DOES NOT EXIST**...", false);
                    return;
                }
        ...
        case "post":
            var fastPostUsageMessage = `**USAGE:**\n\`${PREFIX}fast post <#_MOST_RECENT_ENTRY> <FIELD> <force>\``
                ...
                + "\n\n`<FIELD>`(OPT.): **start; end; fastbreaker; duration; reflection** (includes mood); *Default:* all fields\n(if MULTIPLE `<FIELD>`s: separate by space!)"
                + "\n\n`<force>`(OPT.): type **force** at the end of your command to **skip all of the confimation windows!**";
            fastPostUsageMessage = fn.getMessageEmbed(fastPostUsageMessage, `Fast: Fast Post Help`, "#00FF00");
            ...
            if (args[1] !== undefined) {
                ...
                if (isNaN(args[1])) {
                    ...
                    if (fastView === undefined) {
                        fn.sendErrorMessageAndUsage(message, fastPostHelpMessage, "**FAST DOES NOT EXIST**...", false);
                        return;
                    }
                    fastData = fastCursorToDataArray(fastView[0]);
                    endTimestamp = fastData[1];
        ...
        case "delete":
            ...
            fastDeleteUsage = fn.getMessageEmbed(fastDeleteUsage, `Fast: Fast Delete Help`, "#00FF00");
        ...
        case "edit":
            ...
            fastEditUsage = fn.getMessageEmbed(fastEditUsage, `Fast: Fast Edit Help`, "#00FF00");
            ...
            if (isNaN(args[1]) && args[1].toLowerCase() != "recent") {
                ...
                const fieldToEditAdditionalMessage = `***NO \`<START/END_DATE/TIME>\` EDITING YET*** AND \n**\`fastDuration\`** CAN ONLY BE CHANGED TO **\`now\`**,`
                    + `\n(In Development...)\n\n__**Fast ${pastNumberOfEntriesIndex}:**__\n${showFast}`;
                const fieldToEditTitle = "Fast: Fast Edit Field";
                ...
                let userEdit = await getUserEdit(message, fastFields, fieldToEdit);
                if (userEdit === false) {
                    return;
                }
        ...
        default:
            message.reply(usageHelpMessage);
            break;
    }
}
