...
module.exports = {
  ...
  deleteManyByIDAndConnectedReminders: async function (Model, objectIDs) {
    try {
      ...
      if (!documents) {
        console.log(
          `No ${Model.modelName} documents (${objectIDs}) can be found...`
        );
        return false;
      } else {
        console.log(
          `Deleting ${Model.modelName} documents (${objectIDs}) and it's associated reminders...`
        );
        ...
        documents.forEach(async (document) => {
          if (document._id) {
            ...
            if (reminders.deletedCount === 0) {
              console.log(
                `No reminders associated to ${document._id.toString()}`
              );
            } else
              console.log(
                `Deleted ${
                  reminders.deletedCount
                } reminders associated to ${document._id.toString()}`
              );
          }
        });
        return true;
  ...
  deleteManyAndConnectedReminders: async function (Model, query) {
    try {
      ...
      if (!documents) {
        console.log(
          `No ${Model.modelName} documents found with query: ${query}, can be found...`
        );
        return false;
      } else {
        console.log(
    ...
    try {
      ...
      if (!document) {
        console.log(
          `No ${
            Model.modelName
          } document (${objectID.toString()}) can be found...`
        );
        return false;
      } else {
        console.log(
  ...
  deleteOneAndConnectedReminders: async function (Model, query) {
    try {
      ...
      if (!document) {
        console.log(
          `No ${Model.modelName} document found with query: ${query}, can be found...`
        );
        return false;
      } else {
        console.log(
        ...
        if (deletedCount === 0) {
          ...
          console.log(
            `Deleted ${deletedCount} reminders associated to ${document._id.toString()}`
          );
        return true;
      }
    } catch (err) {
