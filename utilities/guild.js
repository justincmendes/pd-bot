...
module.exports = {
  deleteGuild: async function (guildID, guildName, guildChannelObjectArray) {
    try {
      if (guildID) {
        ...
        console.log(
          `Removing Guild Settings and Reminders from ${
      ...
      if (guildChannelObjectArray)
        if (guildChannelObjectArray.length) {
      ...
      console.log(
        `Unlinking any users voice channel tracking data from this server.`
      );
      return true;
    } catch (err) {
      console.log(err);
  ...
  updateGuilds: async function (bot) {
    try {
      ...
      let guildSettingIDArray = allGuildSettings
        ? allGuildSettings.length
      ...
      if (guildSettingIDArray)
        if (guildSettingIDArray.length) {
  ...
  setupNewGuild: async function (bot, guildID, guildName) {
    try {
      ...
      if (guildObject) {
        console.log(
          `${bot.user.username} is already in ${guildName}! Won't create new instance in Database.`
        );
        return guildObject;
      } else {
        const guildSettings = await this.createGuildSettings(
          guildID,
          "EST",
          true
        );
        if (guildSettings) {
          console.log(
            `${bot.user.username} has joined the server ${guildName}! Saved to Database.`
          );
          return guildSettings;
        } else {
          console.log(
            `There was an error adding ${guildName} to the database.`
          );
          return null;
        }
      }
  ...
  createGuildSettings: async function (
    guildID,
    timezone = "EST",
    daylightSaving = true
  ) {
    try {
      ...
      const daylightOffset = fn.isDaylightSavingTime(
        Date.now() + initialOffset * HOUR_IN_MS,
        timezone,
        daylightSaving
      )
        ? fn.getTimezoneDaylightOffset(timezone)
        : 0;
