...
module.exports = {
  cancelAndDeleteAllTrackReminders: async function (userID) {
    const currentTrackReminders = await Reminder.find({
      userID,
      title: "Voice Channel Tracking",
    });
    if (currentTrackReminders)
      if (currentTrackReminders.length) {
  ...
  setUserTrackingReportReminder: async function (
    bot,
    message,
    PREFIX,
    timezoneOffset,
    ...
    let endTime = await fn.getDateAndTimeEntry(
      ...
      "Voice Channel Tracking: Tracking Report Time of Day",
      true,
      trackEmbedColour
    );
    ...
    let interval = await rm.getInterval(
      ...
      "Voice Channel Tracking: Tracking Report Interval",
      trackEmbedColour,
      HOUR_IN_MS,
      300000,
      60000,
      fn.intervalExamplesOver1Hour
    );
  ...
  userSelectVoiceChannelObject: async function (
    bot,
    message,
    PREFIX,
    voiceChannelArray,
    ...
    voiceChannelArray.forEach((vc, i) => {
      vcList +=
        `\`${i + 1}\` - **${fn.getVoiceChannelNameString(bot, vc)}**` +
  ...
  unlinkVoiceChannelTracking: async function (channelObject) {
    ...
    if (channelObject)
      if (channelObject.type === "voice") {
        ...
        if (allUsersSettings)
          if (allUsersSettings.length) {
            console.log(
              `Unlinking any users' voice channel time tracking from` +
                ...
                if (voiceChannels.length) {
                  voiceChannels.forEach(async (vc, i) => {
                    if (vc.id === channelObject.id) {
                      console.log(
                        `Unlinking ${userSettings.discordTag} from` +
