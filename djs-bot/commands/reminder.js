        ...
        if (reminderCommand === "delete" || reminderCommand === "del" || reminderCommand === "d" || reminderCommand === "remove" || reminderCommand === "rem" || reminderCommand === "r") {
            ...
            if (args[2] !== undefined) {
                ...
                if (deleteType === "past") {
                    ...
                    const multipleDeleteConfirmation = await fn.getUserConfirmation(message, multipleDeleteMessage, forceSkip, `Reminder: Delete Past ${numberArg} Reminders (${sortType})`, 600000);
                ...
                if (deleteType === "many") {
                    ...
                    const sortType = indexByRecency ? "By Recency" : "By End Time";
                    const confirmDeleteMany = await fn.getUserConfirmation(message, deleteConfirmMessage, forceSkip, `Reminder: Delete Reminders ${toDelete} (${sortType})`, 600000)
                    ...
                    if (args[2 + shiftIndex]) {
                        if (args[2 + shiftIndex].toLowerCase() === "past") {
                            ...
                            const multipleDeleteConfirmation = await fn.getUserConfirmation(message, multipleDeleteMessage, forceSkip, `Reminder: Multiple Delete Warning! (${sortType})`);
                            if (!multipleDeleteConfirmation) return;
                            const targetIDs = await reminderCollection.map(reminder => reminder._id);
            ...
            // it will be handled properly later based on the values passed in!
                ...
                if (seeType === "recent") {
                    return message.channel.send(await rm.getMostRecentReminder(bot, authorID, false, userTimezoneOffset, reminderEmbedColour));
                }
                else if (seeType === "all") {
                ...
                // If the first argument after "see" is not past, then it is not a valid call
                    ...
                    message.channel.send(await rm.getMostRecentReminder(bot, authorID, false, userTimezoneOffset, reminderEmbedColour));
                    return message.reply(reminderActionHelpMessage);
                }
                ...
                if (pastFunctionality) {
                    ...
                    if (args[2] !== undefined) {
                        ...
                        let confirmSeeAll = await fn.getUserConfirmation(message, confirmSeeMessage, forceSkip, `Reminder: See ${args[2]} Reminders WARNING! (${sortType})`);
                        ...
                        let confirmSeeAll = await fn.getUserConfirmation(message, confirmSeeAllMessage, forceSkip, "Reminder: See All Reminders WARNING!");
                        if (!confirmSeeAll) return;
                    }
                    ...
                    const reminderEmbed = fn.getMessageEmbed(reminderDataToString, `Reminder: See ${pastNumberOfEntriesIndex} Reminders (${sortType})`, reminderEmbedColour);
                    ...
                    if (args[2 + shiftIndex]) {
                        if (args[2 + shiftIndex].toLowerCase() === "past") {
                            if (args[3 + shiftIndex] !== undefined) {
                                ...
                                const confirmSeePast = await fn.getUserConfirmation(message, confirmSeePastMessage, forceSkip, `Reminder: See ${args[1]} Reminders Past ${entriesToSkip} WARNING! (${sortType})`);
                                ...
                                const reminderEmbed = fn.getMessageEmbed(reminderDataToString, `Reminder: See ${pastNumberOfEntriesIndex} Reminder Past ${entriesToSkip} (${sortType})`, reminderEmbedColour);
                                    ...
                                    case 5:
                                        {
                                            ...
                                            if (typeof userEdit === "boolean") {
                                                ...
                                                if (userEdit === true && reminderData[1] === false) {
                                                    ...
                                                    let duration = await rm.getUserFirstRecurringEndDuration(message, reminderHelpMessage, userTimezoneOffset, userDaylightSavingSetting, true);
                                                ...
                                                // From Repeating to One-Time
                                                    ...
                                                    let duration = await rm.getUserFirstRecurringEndDuration(message, reminderHelpMessage, userTimezoneOffset, userDaylightSavingSetting, false);
                                                    console.log({ duration })
                                                    if (!duration && duration !== 0) {
                                                        continueEdit = true;
                                                        break;
