...
module.exports = {
    ...
    run: async function run(bot, message, commandUsed, args, PREFIX,
        ...
        let repeatUsageMessage = `**USAGE** (Recurring Reminder)\n\`${PREFIX}${commandUsed} <ACTION?> <force?>\``
            ...
            + `If you want to set a one-time reminder, try \`${PREFIX}reminder <force?>\``
            + `\n\n*__ALIASES:__* **${this.name} - ${this.aliases.join('; ')}**`;
        repeatUsageMessage = fn.getMessageEmbed(repeatUsageMessage, "Recurring Reminder: Help", repeatEmbedColour);
        ...
        if (reminderCommand === "delete" || reminderCommand === "del" || reminderCommand === "d" || reminderCommand === "remove" || reminderCommand === "rem" || reminderCommand === "r") {
            ...
            reminderDeleteUsageMessage = fn.getMessageEmbed(reminderDeleteUsageMessage, "Recurring Reminder: Delete Help", repeatEmbedColour);
            ...
            if (reminderIndex) {
                ...
                if (!totalReminderNumber) {
                    return message.reply(`**NO RECURRING REMINDERS**... try \`${PREFIX}${commandUsed} help\` to set one up!`);
                }
            }
            ...
            if (args[2] !== undefined) {
                ...
                if (deleteType === "many") {
                    ...
                    for (i = 0; i < toDelete.length; i++) {
                        ...
                        else reminderDocument = await rm.getOneReminderByEndTime(authorID, toDelete[i] - 1, true);
            ...
            if (isNaN(args[1])) {
                ...
                var reminderDocument;
                if (indexByRecency) reminderDocument = await rm.getOneReminderByRecency(authorID, pastNumberOfEntriesIndex - 1, true);
                else reminderDocument = await rm.getOneReminderByEndTime(authorID, pastNumberOfEntriesIndex - 1, true);
                if (!reminderDocument) {
            ...
            reminderSeeUsageMessage = fn.getMessageEmbed(reminderSeeUsageMessage, "Recurring Reminder: See Help", repeatEmbedColour);
            if (reminderIndex) {
                if (reminderIndex === "help") {
            ...
            // it will be handled properly later based on the values passed in!
                ...
                if (seeType === "recent") {
                    return message.channel.send(await rm.getMostRecentReminder(bot, authorID, true, timezoneOffset, repeatEmbedColour));
                }
                else if (seeType === "all") {
                ...
                // If the first argument after "see" is not past, then it is not a valid call
                    ...
                    message.channel.send(await rm.getMostRecentReminder(bot, authorID, true, timezoneOffset, repeatEmbedColour));
                    return message.reply(reminderActionHelpMessage);
                }
                ...
                if (pastFunctionality) {
                    ...
                    var reminderDocument;
                    if (indexByRecency) reminderDocument = await fn.getEntriesByRecency(Reminder, { userID: authorID, isRecurring: true }, 0, pastNumberOfEntriesIndex);
                    else reminderDocument = await fn.getEntriesByEarliestEndTime(Reminder, { userID: authorID, isRecurring: true }, 0, pastNumberOfEntriesIndex);
                    ...
                    await fn.sendPaginationEmbed(bot, message.channel.id, authorID, fn.getEmbedArray(reminderDataToStringArray, `Recurring Reminder: See ${pastNumberOfEntriesIndex} Reminders (${sortType})`, true, true, repeatEmbedColour));
                    return;
                }
                    ...
                    if (args[2 + shiftIndex]) {
                        if (args[2 + shiftIndex].toLowerCase() === "past") {
                            if (args[3 + shiftIndex] !== undefined) {
                                ...
                                var reminderDocument;
                                if (indexByRecency) reminderDocument = await fn.getEntriesByRecency(Reminder, { userID: authorID, isRecurring: true }, entriesToSkip, pastNumberOfEntriesIndex);
                                else reminderDocument = await fn.getEntriesByEarliestEndTime(Reminder, { userID: authorID, isRecurring: true }, entriesToSkip, pastNumberOfEntriesIndex);
                                console.log({ reminderView: reminderDocument });
                                ...
                                await fn.sendPaginationEmbed(bot, message.channel.id, authorID, fn.getEmbedArray(reminderDataToStringArray, `Recurring Reminder: See ${pastNumberOfEntriesIndex} Reminder Past ${entriesToSkip} (${sortType})`, true, true, repeatEmbedColour));
                ...
                const reminderEmbed = fn.getEmbedArray(reminderDataToString, `Recurring Reminder: See Reminder ${pastNumberOfEntriesIndex} (${sortType})`, true, true, repeatEmbedColour);
        ...
        // EDIT
            ...
            let reminderEditUsageMessage = `**USAGE:**\n\`${PREFIX}${commandUsed} ${reminderCommand} <#_MOST_RECENT_ENTRY> <recent?> <force?>\``
                ...
                + "\n\n`<force?>`(OPT.): type **force** at the end of your command to **skip all of the confirmation windows!**";
            reminderEditUsageMessage = fn.getMessageEmbed(reminderEditUsageMessage, `Recurring Reminder: Edit Help`, repeatEmbedColour);
            if (reminderIndex) {
                if (reminderIndex === "help") {
                ...
                if (isNaN(reminderIndex) && reminderIndex !== "recent") {
                    ...
                    do {
                        ...
                        const selectedField = await fn.getUserSelectedObject(bot, message, PREFIX,
                            ...
                            repeatEmbedColour, 600000, 0, fieldToEditAdditionalMessage);
                        ...
                        switch (fieldToEditIndex) {
                            ...
                            case 5:
                                ...
                                userEdit = await fn.getUserEditBoolean(bot, message, PREFIX, fieldToEdit, reminderEditMessagePrompt,
                                    ['⌚', '1️⃣'], reminderType, true, repeatEmbedColour);
                                break;
                            case 6:
                                if (isRecurring === true) {
                                            ...
                                            if (typeof userEdit === "boolean") {
                                                ...
                                                if (userEdit === true && isRecurring === false) {
                                                    ...
                                                    interval = await rm.getEditInterval(bot, message, PREFIX, timezoneOffset, daylightSavingsSetting,
                                                        ...
                                                        reminderType, repeatEmbedColour);
                                                    if (!interval) {
                                                        continueEdit = true;
                                                        break;
                                                    }
