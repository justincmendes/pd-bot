...
const reminderEmbedColour = fn.repeatReminderEmbedColour;
...
module.exports = {
    ...
    run: async function run(bot, message, commandUsed, args, PREFIX, forceSkip) {
        ...
        if (reminderCommand === "delete" || reminderCommand === "del" || reminderCommand === "d" || reminderCommand === "remove" || reminderCommand === "rem" || reminderCommand === "r") {
            ...
            reminderDeleteUsageMessage = fn.getMessageEmbed(reminderDeleteUsageMessage, "Repeat Reminder: Delete Help", reminderEmbedColour);
            ...
            if (args[2] !== undefined) {
                ...
                if (deleteType === "past") {
                    ...
                    const showReminders = rm.multipleRemindersToString(bot, message, reminderCollection, numberArg, userTimezoneOffset);
                    ...
                    const multipleDeleteConfirmation = await fn.getUserConfirmation(message, multipleDeleteMessage, forceSkip, `Repeat Reminder: Delete Past ${numberArg} Reminders (${sortType})`, 600000);
                ...
                if (deleteType === "many") {
                    ...
                    for (i = 0; i < toDelete.length; i++) {
                        var reminderView;
                        if (indexByRecency) {
                            reminderView = await rm.getOneReminderByRecency(authorID, toDelete[i] - 1, true);
                        }
                        else {
                            reminderView = await rm.getOneReminderByEndTime(authorID, toDelete[i] - 1, true);
                        }
                        var reminderData;
                        if (toDelete[i] === 1) {
                            reminderData = rm.reminderDocumentToDataArray(reminderView, userTimezoneOffset, true);
                        }
                        else {
                            reminderData = rm.reminderDocumentToDataArray(reminderView);
                        }
                        reminderTargetIDs.push(reminderView._id);
                        reminderDataToString = `\n__**Reminder ${toDelete[i]}:**__\n` + rm.reminderDataArrayToString(bot, reminderData, userTimezoneOffset);
                        deleteConfirmMessage = deleteConfirmMessage + reminderDataToString + "\n";
                    }
                    deleteConfirmMessage = `Are you sure you want to **delete reminders ${toDelete.toString()}?:**\n` + deleteConfirmMessage;
                    const sortType = indexByRecency ? "By Recency" : "By End Time";
                    const confirmDeleteMany = await fn.getUserConfirmation(message, deleteConfirmMessage, forceSkip, `Repeat Reminder: Delete Reminders ${toDelete} (${sortType})`, 600000)
                    ...
                    if (args[2 + shiftIndex]) {
                        if (args[2 + shiftIndex].toLowerCase() === "past") {
                            ...
                            const showReminders = rm.multipleRemindersToString(bot, message, reminderCollection, pastNumberOfEntries, userTimezoneOffset, skipEntries);
                            ...
                            const multipleDeleteConfirmation = await fn.getUserConfirmation(message, multipleDeleteMessage, forceSkip, `Repeat Reminder: Multiple Delete Warning! (${sortType})`);
                            if (!multipleDeleteConfirmation) return;
                            const targetIDs = await reminderCollection.map(reminder => reminder._id);
            ...
            if (isNaN(args[1])) {
                ...
                if (deleteType == "recent") {
                    ...
                    const deleteIsConfirmed = await fn.getUserConfirmation(message, deleteConfirmMessage, forceSkip, `Repeat Reminder: Delete Recent Reminder`, 300000)
                ...
                var reminderView;
                if (indexByRecency) reminderView = await rm.getOneReminderByRecency(authorID, pastNumberOfEntriesIndex - 1, true);
                else reminderView = await rm.getOneReminderByEndTime(authorID, pastNumberOfEntriesIndex - 1, true);
                if (!reminderView) {
                ...
                const deleteConfirmMessage = `Are you sure you want to **delete Reminder ${pastNumberOfEntriesIndex}?:**\n\n__**Reminder ${pastNumberOfEntriesIndex}:**__\n` +
                    rm.reminderDataArrayToString(bot, reminderData, userTimezoneOffset);
                const deleteConfirmation = await fn.getUserConfirmation(message, deleteConfirmMessage, forceSkip, `Repeat Reminder: Delete Reminder ${pastNumberOfEntriesIndex} (${sortType})`, 300000);
            ...
            reminderSeeUsageMessage = fn.getMessageEmbed(reminderSeeUsageMessage, "Repeat Reminder: See Help", reminderEmbedColour);
            if (reminderIndex) {
                if (reminderIndex === "help") {
            ...
            // it will be handled properly later based on the values passed in!
                ...
                // If the first argument after "see" is not past, then it is not a valid call
                    ...
                    message.channel.send(await rm.getMostRecentReminder(bot, authorID, true, userTimezoneOffset, reminderEmbedColour));
                    return message.reply(reminderActionHelpMessage);
                }
                ...
                if (pastFunctionality) {
                    ...
                    if (args[2] !== undefined) {
                        ...
                        let confirmSeeAll = await fn.getUserConfirmation(message, confirmSeeMessage, forceSkip, `Repeat Reminder: See ${args[2]} Reminders WARNING! (${sortType})`);
                        ...
                        let confirmSeeAll = await fn.getUserConfirmation(message, confirmSeeAllMessage, forceSkip, "Repeat Reminder: See All Reminders WARNING!");
                        if (!confirmSeeAll) return;
                    }
                    ...
                    var reminderView;
                    if (indexByRecency) reminderView = await fn.getEntriesByRecency(Reminder, { userID: authorID, isRecurring: true }, 0, pastNumberOfEntriesIndex);
                    ...
                    const reminderDataToString = rm.multipleRemindersToString(bot, message, reminderView, pastNumberOfEntriesIndex, userTimezoneOffset);
                    const reminderEmbed = fn.getMessageEmbed(reminderDataToString, `Repeat Reminder: See ${pastNumberOfEntriesIndex} Reminders (${sortType})`, reminderEmbedColour);
                        ...
                        if (args[2 + shiftIndex].toLowerCase() === "past") {
                            if (args[3 + shiftIndex] !== undefined) {
                                ...
                                if (!isNaN(args[3 + shiftIndex])) {
                                    ...
                                    entriesToSkip = await rm.getRecentReminderIndex(authorID, true);
                                }
                                else return message.reply(reminderActionHelpMessage);
                                ...
                                const confirmSeePast = await fn.getUserConfirmation(message, confirmSeePastMessage, forceSkip, `Repeat Reminder: See ${args[1]} Reminders Past ${entriesToSkip} WARNING! (${sortType})`);
                                ...
                                console.log({ reminderView });
                                const reminderDataToString = rm.multipleRemindersToString(bot, message, reminderView, pastNumberOfEntriesIndex, userTimezoneOffset, entriesToSkip);
                                const reminderEmbed = fn.getMessageEmbed(reminderDataToString, `Repeat Reminder: See ${pastNumberOfEntriesIndex} Reminder Past ${entriesToSkip} (${sortType})`, reminderEmbedColour);
                ...
                const reminderEmbed = fn.getMessageEmbed(reminderDataToString, `Repeat Reminder: See Reminder ${pastNumberOfEntriesIndex} (${sortType})`, reminderEmbedColour);
                message.channel.send(reminderEmbed);
            }
        }
        ...
        // EDIT
            ...
            let reminderEditUsageMessage = `**USAGE:**\n\`${PREFIX}${commandUsed} ${reminderCommand} <#_MOST_RECENT_ENTRY> <recent?> <force?>\``
                ...
                + "\n\n`<force?>`(OPT.): type **force** at the end of your command to **skip all of the confirmation windows!**";
            reminderEditUsageMessage = fn.getMessageEmbed(reminderEditUsageMessage, `Repeat Reminder: Edit Help`, reminderEmbedColour);
            if (reminderIndex) {
                if (reminderIndex === "help") {
                    return message.channel.send(reminderDeleteUsageMessage);
                }
                ...
                if (isNaN(reminderIndex) && reminderIndex !== "recent") {
                    ...
                    do {
                        ...
                        continueEdit = false;
                        reminderData = rm.reminderDocumentToDataArray(reminderView);
                        showReminder = rm.reminderDataArrayToString(bot, reminderData, userTimezoneOffset);
                        // Field the user wants to edit
                        const fieldToEditInstructions = "**Which field do you want to edit?:**";
                        const fieldToEditAdditionalMessage = `__**Reminder ${pastNumberOfEntriesIndex} (${sortType}):**__\n${showReminder}`;
                        const fieldToEditTitle = `Repeat Reminder: Edit Field`;
                        ...
                        if (userEdit === false) return;
                            ...
                            if (fieldToEditIndex === 2 || fieldToEditIndex === 3) {
                                ...
                                console.log({ userEdit });
                                reminderData[fieldToEditIndex + 3] = fn.timeCommandHandlerToUTC(userEdit, timestamp, userTimezoneOffset, userDaylightSavingSetting)
                                    ...
                                    case 0:
                                        {
                                            ...
                                            if (validTypes.includes(userType)) {
                                                let removeConnectedDocsConf = await fn.getUserConfirmation(message,
                                                    ...
                                                    forceSkip, "Repeat Reminder: Change Type Confirmation", 90000);
                                    ...
                                    case 5:
                                        {
                                            ...
                                            if (typeof userEdit === "boolean") {
                                                ...
                                                if (userEdit === true && reminderData[1] === false) {
                                                    ...
                                                    let confirmCreation = await fn.getUserConfirmation(message, confirmCreationMessage, forceSkip, "Recurring Repeat Reminder: Confirm Creation", 180000);
                                                    if (!confirmCreation) {
                                                        continueEdit = true;
                                                        break;
                                                    }
                                                ...
                                                // From Repeating to One-Time
                                                    ...
                                                    let confirmCreationMessage = `Are you sure you want to set the following **one-time reminder** to send -\n**in ${channel} after ${fn.millisecondsToTimeString(duration)}**`
                                                        + `\n\n${reminderData[7]}`;
                                                    let confirmCreation = await fn.getUserConfirmation(message, confirmCreationMessage, forceSkip, "Repeat Reminder: Confirm Creation", 180000);
                                                    if (!confirmCreation) {
                                                        continueEdit = true;
                                                        break;
                                                    }
                            ...
                            if (!continueEdit) {
                                try {
                                    ...
                                    if (reminderView) {
                                        ...
                                        continueEdit = await fn.getUserConfirmation(message, continueEditMessage, forceSkip, `Repeat Reminder: Continue Editing Reminder ${pastNumberOfEntriesIndex}?`, 300000);
                                    }
                                    else {
