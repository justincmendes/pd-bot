...
async function sendGeneratedTemplate(bot, message, numberOfUsers, namesForTemplate, withMarkdown = true, templateEmbedColour = mastermindEmbedColour) {
    ...
    await fn.sendPaginationEmbed(bot, message.channel.id, message.author.id, fn.getEmbedArray(
        templateArray, "Mastermind: Weekly Reflection And Goals Template", true,
        "Mastermind Weekly Reflection and Weekly Goals Template", templateEmbedColour));
}
...
async function setUserMastermindReminder(bot, message, PREFIX, timezoneOffset, daylightSaving, mastermindDocument) {
    ...
    const mastermindReminders = await getCurrentMastermindReminders(message.author.id);
    ...
    await setMastermindWeeklyGoalReminder(bot, message.author.id, endTime,
    ...
    run: async function run(bot, message, commandUsed, args, PREFIX,
        ...
        if (mastermindCommand === "start" || mastermindCommand === "st" || mastermindCommand === "s" || mastermindCommand === "set" || mastermindCommand === "create"
            ...
            if (userWantsTemplate) {
                let observations = await fn.getMultilineEntry(bot, message, PREFIX, "**__Look back at the previous week ↩:__**"
                ...
                const areaOfLifeIndex = await fn.userSelectFromList(bot, message, PREFIX, areasOfLifeList, areasOfLife.length, "**__Which area of life needs the most attention this week? 🌱__**",
                ...
                do {
                    ...
                    const weeklyGoalType = await fn.userSelectFromList(bot, message, PREFIX, `${areasOfLifeList}\n\n${goalDescriptionString}`, areasOfLife.length,
                ...
                let entry = await fn.getMultilineEntry(bot, message, PREFIX, "**Enter your mastermind entry:**",
            ...
            if (args[2] !== undefined) {
                ...
                if (deleteType === "many") {
                    ...
                    if (confirmDeleteMany) {
                        ...
                        await del.deleteManyByIDAndConnectedReminders(Mastermind, mastermindTargetIDs);
                        return;
                    }
                    else return;
                }
            ...
            if (isNaN(args[1])) {
                ...
                if (deleteType === "recent") {
                    ...
                    const mastermindEmbed = fn.getEmbedArray(`__**Mastermind ${mastermindIndex}:**__\n${mastermindDocumentToString(bot, mastermindView)}`,
                        `Mastermind: Delete Recent Entry`, true, false, mastermindEmbedColour);
                    ...
                    if (deleteIsConfirmed) {
                        await del.deleteOneByIDAndConnectedReminders(Mastermind, mastermindTargetID);
                        return;
                    }
                }
                ...
                const mastermindEmbed = fn.getEmbedArray(`__**Mastermind ${pastNumberOfEntriesIndex}:**__\n${mastermindDocumentToString(bot, mastermindView)}`,
                    `Mastermind: Delete Entry ${pastNumberOfEntriesIndex} (${sortType})`, true, false, mastermindEmbedColour);
            ...
            // it will be handled properly later based on the values passed in!
                ...
                if (pastFunctionality) {
                    ...
                    await fn.sendPaginationEmbed(bot, message.channel.id, authorID, fn.getEmbedArray(
                        mastermindStringArray, `Mastermind: See ${pastNumberOfEntriesIndex} Entries (${sortType})`,
                        true, `Mastermind Reflections ${fn.timestampToDateString(
                    ...
                    if (args[2 + shiftIndex]) {
                        if (args[2 + shiftIndex].toLowerCase() === "past") {
                            if (args[3 + shiftIndex] !== undefined) {
                                ...
                                await fn.sendPaginationEmbed(bot, message.channel.id, authorID, fn.getEmbedArray(
                                    mastermindStringArray, `Mastermind: See ${pastNumberOfEntriesIndex} Entries Past ${entriesToSkip} (${sortType})`,
                                    true, `Mastermind Reflections ${fn.timestampToDateString(
                                        Date.now() + timezoneOffset * HOUR_IN_MS, false, false, true, true
                                    )}`, mastermindEmbedColour));
                                return;
                ...
                const mastermindEmbed = fn.getEmbedArray(mastermindString, `Mastermind: See Entry ${pastNumberOfEntriesIndex} (${sortType})`,
                    true, `Mastermind Reflection ${fn.timestampToDateString(
                        Date.now() + timezoneOffset * HOUR_IN_MS, false, false, true, true
                    )}`, mastermindEmbedColour);
                ...
                if (isNaN(mastermindType) && mastermindType !== "recent") {
                    ...
                    do {
                        ...
                        const selectedField = await fn.getUserSelectedObject(bot, message, PREFIX,
                            fieldToEditInstructions, fieldToEditTitle, mastermindFields, "", false,
                            mastermindEmbedColour, 600000, 0, fieldToEditAdditionalMessage);
                            ...
                            case 6:
                                {
                                    ...
                                    let goalIndex = await fn.userSelectFromList(bot, message, PREFIX, fn.goalArrayToString(goalsArray, "Weekly", true, true, true), goalsArray.length, `\n**__Please enter the \`number\` of the goal you'd like to change__:**`,
            ...
            if (args[1] !== undefined) {
                ...
                const sortType = indexByRecency ? "By Recency" : "By Date Created";
                const targetChannel = await fn.getTargetChannel(bot, message, PREFIX,
                    `Mastermind ${sortType}`, forceSkip, true, false, true, mastermindEmbedColour);
