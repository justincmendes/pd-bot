      ...
      do {
        ...
        await habitDocument
          ...
          .then(async (result) => {
            ...
            totalHabitNumber++;
            message.reply(
              `**Habit ${await getHabitIndexByFunction(
                authorID,
                habitDocument._id,
                totalHabitNumber,
                false,
      ...
      if (args[2 + archiveShift] !== undefined) {
        ...
        if (deleteType === "many") {
          ...
          for (let i = 0; i < toDelete.length; i++) {
            ...
            habitArray.push(
              `__**Habit ${toDelete[i]}:**__ ${await habitDocumentToString(
                bot,
                habitView,
                true,
                true,
      ...
      if (isNaN(args[1 + archiveShift])) {
        ...
        if (deleteType === "recent") {
          ...
          const habitEmbed = fn.getEmbedArray(
            `__**Habit ${habitIndex}:**__ ${await habitDocumentToString(
              bot,
              habitView,
              true,
              true,
                    ...
                    case 0:
                      ...
                      if (!userEdit) {
                        ...
                        const confirmOverwiteLog = await fn.getUserConfirmation(
                          ...
                          false,
                          `${type}: Overwrite Log (Timestamp Conflict)`,
                          300000
                        );
        ...
        if (userHabits.length) {
          ...
          for (const habit of targetHabits) {
            ...
            habitStringArray.push(
              `**__Habit ${habitIndex}:__** ${await habitDocumentToString(
                bot,
                habit,
                true,
                false,
