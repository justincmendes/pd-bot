...
function multipleLogsToStringArray(message, logArray, numberOfLogs,
    entriesToSkip = 0, toString = false) {
    var logsToString = new Array();
    console.log({ numberOfLogs });
    for (let i = 0; i < numberOfLogs; i++) {
        if (logArray[i] === undefined) {
            numberOfLogs = i;
            fn.sendErrorMessage(message, `**LOGS ${i + entriesToSkip + 1}**+ ONWARDS DO NOT EXIST...`);
            break;
        }
...
async function getRecentHabit(bot, userID, isArchived, embedColour, showConnectedGoal = false,
    ...
    const habitEmbed = fn.getMessageEmbed(recentHabitToString, `Log: See Recent Habit`, embedColour);
    return habitEmbed;
}

async function getHabitIndexByFunction(userID, habitID, totalLogs, archived, getOneHabit) {
    let i = 0;
    while (true) {
...
async function getLogsByCreatedAt(userID, entryIndex, numberOfEntries = 1, archived = undefined) {
    try {
...
module.exports = {
    ...
    run: async function run(bot, message, commandUsed, args, PREFIX,
        ...
        let logUsageMessage = `**USAGE**\n\`${PREFIX}${commandUsed} <ACTION>\``
            + "\n\n\`<ACTION>\`: **add/start; see; delete; edit**"
            + `\n\n*__ALIASES:__* **${this.name} - ${this.aliases.join('; ')}**`;
        logUsageMessage = fn.getMessageEmbed(logUsageMessage, "Log: Help", habitEmbedColour);
        ...
        // Allow the user to specify the timestamp of the log! (now, today, tomorrow, yesterday)
        else if (logCommand === "start" || logCommand === "create" || logCommand === "s" || logCommand === "set"
            || logCommand === "st" || logCommand === "c" || logCommand === "make" || logCommand === "m"
            || logCommand === "add" || logCommand === "a") {
            ...
            logStartUsageMessage = fn.getMessageEmbed(logStartUsageMessage, `Log: Start Help`, habitEmbedColour);
            ...
            var logArray;
            if (indexByRecency) logArray = await Habit.find({ archived: isArchived, userID: authorID }).sort({ _id: -1 });
            else logArray = await Habit.find({ archived: isArchived, userID: authorID }).sort({ createdAt: +1 });
            if (!logArray.length) return message.reply(`**No ${isArchived ? "archived " : ""}habits** were found... Try \`${PREFIX}${commandUsed} help\` for help!`);

            do {
                let targetHabit = await fn.getUserSelectedObject(bot, message, PREFIX, "__**Which habit would you like to log?**__",
                    `Log${isArchived ? " Archive" : ""}: Select Habit To Log`, logArray, "description", false, habitEmbedColour, 600000);
                ...
                let habitLog = await fn.userSelectFromList(bot, message, PREFIX, checkMissedSkipList, 3,
                    ...
                    `Log${isArchived ? " Archive" : ""}: Log`, habitEmbedColour, 600000, 0);
                ...
                const confirmReflection = await fn.getUserConfirmation(bot, message, PREFIX,
                    ...
                    forceSkip, `Log${isArchived ? " Archive" : ""}: Write Reflection Message? (Optional)`, 300000);
                ...
                const confirmEnd = await fn.getUserConfirmation(bot, message, PREFIX,
                    ...
                    forceSkip, `Log${isArchived ? " Archive" : ""}: Confirm Log?`);
                if (confirmEnd) {
        ...
        else if (logCommand === "delete" || logCommand === "del" || logCommand === "d"
            || logCommand === "remove" || logCommand === "rem" || logCommand === "r") {
            /**
             * Allow them to delete any habits - archived or not
             */

            let habitDeleteUsageMessage = hb.getHabitReadOrDeleteHelp(PREFIX, commandUsed, logCommand);
            habitDeleteUsageMessage = fn.getMessageEmbed(habitDeleteUsageMessage, "Log: Delete Help", habitEmbedColour);
            ...
            if (args[2 + archiveShift] !== undefined) {
                ...
                if (deleteType === "past") {
                    ...
                    else habitCollection = await getLogsByCreatedAt(authorID, 0, numberArg, isArchived);
                    const habitArray = fn.getEmbedArray(multipleLogsToStringArray(
                        message, habitCollection, numberArg, 0, false), '', true, false, habitEmbedColour);
                    ...
                    const multipleDeleteConfirmation = await fn.getPaginatedUserConfirmation(bot, message, PREFIX, habitArray, multipleDeleteMessage, forceSkip,
                        `Log${isArchived ? ` Archive` : ""}: Delete Past ${numberArg} Logs (${sortType})`, 600000);
                    if (!multipleDeleteConfirmation) return;
                    const targetIDs = await habitCollection.map(entry => entry._id);
                    console.log(`Deleting ${authorUsername}'s (${authorID}) Past ${numberArg} Logs (${sortType})`);
                ...
                if (deleteType === "many") {
                    ...
                    for (let i = 0; i < toDelete.length; i++) {
                        ...
                        const habitLogs = await Log.find({ connectedDocument: habitView._id });
                        if (habitLogs) if (habitLogs.length) {
                            logArray.push(`__**Habit ${toDelete[i]}:**__ ${hb.habitDocumentDescription(habitView)}`)
                    ...
                    const sortType = indexByRecency ? "By Recency" : "By Date Created";
                    logArray = fn.getEmbedArray(logArray, '', true, false, habitEmbedColour);
                    const confirmDeleteMany = await fn.getPaginatedUserConfirmation(bot, message, PREFIX, logArray, deleteConfirmMessage,
                        forceSkip, `Log${isArchived ? ` Archive` : ""}: Delete Logs ${toDelete} (${sortType})`, 600000);
                    if (confirmDeleteMany) {
                        console.log(`Deleting ${authorID}'s Logs ${toDelete} (${sortType})`);
                    ...
                    if (args[2 + archiveShift + shiftIndex]) {
                        if (args[2 + archiveShift + shiftIndex].toLowerCase() === "past") {
                            ...
                            else habitCollection = await getLogsByCreatedAt(authorID, skipEntries, pastNumberOfEntries, isArchived);
                            const habitArray = fn.getEmbedArray(multipleLogsToStringArray(
                                message, habitCollection, pastNumberOfEntries, skipEntries, false),
                            ...
                            const multipleDeleteConfirmation = await fn.getPaginatedUserConfirmation(bot, message, PREFIX, habitArray, multipleDeleteMessage,
                                forceSkip, `Log${isArchived ? ` Archive` : ""}: Multiple Delete Warning! (${sortType})`);
            ...
            const noLogsMessage = `**NO ${isArchived ? "ARCHIVED " : ""}HABITS**... try \`${PREFIX}${commandUsed} start help\``;
            if (isNaN(args[1 + archiveShift])) {
                ...
                if (deleteType === "recent") {
                    ...
                    const habitEmbed = fn.getEmbedArray(hb.logDocumentToString(habitView),
                        `Log${isArchived ? " Archive" : ""}: Delete Recent Habit`, true, false, habitEmbedColour);
                    ...
                    const deleteIsConfirmed = await fn.getPaginatedUserConfirmation(bot, message, PREFIX, habitEmbed, deleteConfirmMessage, forceSkip,
                        `Log${isArchived ? " Archive" : ""}: Delete Recent Habit`, 600000);
                    ...
                    let confirmDeleteAll = await fn.getUserConfirmation(bot, message, PREFIX, confirmDeleteAllMessage, forceSkip, `Log${isArchived ? ` Archive` : ""}: Delete All Logs WARNING!`);
                    ...
                    const finalDeleteAllMessage = "Are you reaaaallly, really, truly, very certain you want to delete **ALL OF YOUR HABITS ON RECORD**?\n\nYou **cannot UNDO** this!"
                        + `\n\n*(I'd suggest you* \`${PREFIX}${commandUsed} see all\` *or* \`${PREFIX}${commandUsed} archive all\` *first)*`;
                    let finalConfirmDeleteAll = await fn.getUserConfirmation(bot, message, PREFIX, finalDeleteAllMessage, `Log${isArchived ? ` Archive` : ""}: Delete ALL Logs FINAL Warning!`);
                    if (!finalConfirmDeleteAll) return;

                    console.log(`Deleting ALL OF ${authorUsername}'s (${authorID}) Recorded Logs`);
                ...
                const habitEmbed = fn.getEmbedArray(`__**Habit ${pastNumberOfEntriesIndex}:**__ ${await habitDocumentToString(bot, habitView, true, true, true, true)}`,
                    `Log${isArchived ? ` Archive` : ""}: Delete Habit ${pastNumberOfEntriesIndex} (${sortType})`, true, false, habitEmbedColour);
                ...
                const deleteConfirmation = await fn.getPaginatedUserConfirmation(bot, message, PREFIX, habitEmbed, deleteConfirmMessage, forceSkip,
                    `Log${isArchived ? ` Archive` : ""}: Delete Habit ${pastNumberOfEntriesIndex} (${sortType})`, 600000);
            ...
            let habitSeeUsageMessage = hb.getHabitReadOrDeleteHelp(PREFIX, commandUsed, logCommand);
            habitSeeUsageMessage = fn.getMessageEmbed(habitSeeUsageMessage, `Log${isArchived ? ` Archive` : ""}: See Help`, habitEmbedColour);
            ...
            if (!seeCommands.includes(logType) && !archiveRegex.test(logType) && isNaN(logType)) {
                return message.reply(habitActionHelpMessage);
            }
            ...
            // it will be handled properly later based on the values passed in!
                ...
                if (pastFunctionality) {
                    ...
                    if (args[2 + archiveShift] !== undefined) {
                        ...
                        let confirmSeeLogs = await fn.getUserConfirmation(bot, message, PREFIX, confirmSeeMessage, forceSkip, `Log${isArchived ? ` Archive` : ""}: See ${args[2 + archiveShift]} Logs(${sortType})`);
                        ...
                        let confirmSeeAll = await fn.getUserConfirmation(bot, message, PREFIX, confirmSeeAllMessage, forceSkip, `Log${isArchived ? ` Archive` : ""}: See All Logs`);
                    ...
                    else habitView = await getLogsByCreatedAt(authorID, 0, habitIndex, isArchived);
                    console.log({ habitView, pastNumberOfEntriesIndex: habitIndex });
                    const habitArray = multipleLogsToStringArray(message, habitView, habitIndex, 0, false);
                    await fn.sendPaginationEmbed(bot, message.channel.id, authorID, fn.getEmbedArray(
                        habitArray, `Log${isArchived ? ` Archive` : ""}: See ${habitIndex} Logs(${sortType})`,
                        true, `Logs ${fn.timestampToDateString(
                    ...
                    if (args[2 + archiveShift + shiftIndex]) {
                        if (args[2 + archiveShift + shiftIndex].toLowerCase() === "past") {
                            if (args[3 + archiveShift + shiftIndex] !== undefined) {
                                ...
                                const confirmSeePast = await fn.getUserConfirmation(bot, message, PREFIX, confirmSeePastMessage, forceSkip, `Log${isArchived ? ` Archive` : ""}: See ${args[1 + archiveShift]} Logs Past ${entriesToSkip} (${sortType})`);
                                ...
                                else habitView = await getLogsByCreatedAt(authorID, entriesToSkip, habitIndex, isArchived);
                                console.log({ habitView });
                                const habitStringArray = multipleLogsToStringArray(message, habitView,
                                    habitIndex, entriesToSkip, false);
                                await fn.sendPaginationEmbed(bot, message.channel.id, authorID, fn.getEmbedArray(
                                    habitStringArray, `Log${isArchived ? ` Archive` : ""}: See ${habitIndex} Logs Past ${entriesToSkip} (${sortType})`,
                                    true, `Logs ${fn.timestampToDateString(
                                        Date.now() + timezoneOffset * HOUR_IN_MS, false, false, true, true
                                    )}`, habitEmbedColour));
                                return;
                ...
                const habitEmbed = fn.getEmbedArray(habitString, `Log${isArchived ? ` Archive` : ""}: See Habit ${habitIndex} (${sortType})`,
        ...
        else if (logCommand === "edit" || logCommand === "change" || logCommand === "ed"
            || logCommand === "ch" || logCommand === "c") {
            let habitEditUsageMessage = `** USAGE:**\n\`${PREFIX}${commandUsed} ${logCommand} <archive?> <HABIT #> <recent?> <force?>\``
                ...
                + "\n\n`<force?>`(OPT.): type **force** at the end of your command to **skip all of the confirmation windows!**";
            habitEditUsageMessage = fn.getMessageEmbed(habitEditUsageMessage, `Log: Edit Help`, habitEmbedColour);
            if (logType) {
                if (logType === "help") {
                ...
                if (isNaN(logType) && logType !== "recent" && !archiveRegex.test(logType)) {
                    return message.reply(habitActionHelpMessage);
                }
            }
            logType = isArchived ? args[2] ? args[2].toLowerCase() : false : logType;
            if (logType) {
                var habitIndex;
                if (logType === "recent") {
                ...
                do {
                    ...
                    switch (fieldToEditIndex) {
                        ...
                        case 9:
                            ...
                            switch (tier) {
                                case 1: noMoreStreakLogsAtTier = totalStreakNumber >= streakHabitMax && tier === 1;
                                    break;
                                case 3: noMoreStreakLogsAtTier = false;
                                    break;
                                default: noMoreStreakLogsAtTier = true;
                                    break;
                            }
                            userEdit = await fn.userSelectFromList(bot, message, PREFIX, `\n\`1\` - **No** ⛔${noMoreStreakLogsAtTier ? "" : `\n\`2\` - **Yes, As a Streak** (Every Reset Time) 🔄`}`
                                + `${hasCountGoal ? `\n\`3\` - **Yes, Based on Count Goal** (When goal is reached after logging habit) 🔢` : ""}`, (hasCountGoal ? 2 : 1) + (noMoreStreakLogsAtTier ? 0 : 1),
                                `**__Do you want the habit to automatically log/complete?__**\n(You can still manually log/edit your entries)`
                                + `\n${noMoreStreakLogsAtTier ? `P.S. You've reached your **maximum number of streak habits (${streakHabitMax}) for your tier level (${tier})**` : ""}`,
                    ...
                    if (!continueEdit) {
                        try {
                            ...
                            if (habitDocument) {
                                ...
                                continueEdit = await fn.getUserConfirmation(bot, message, PREFIX, continueEditMessage, forceSkip, `Log${isArchived ? " Archive" : ""}: Continue Editing Habit ${habitIndex}?`, 300000);
                            }
                            else {
