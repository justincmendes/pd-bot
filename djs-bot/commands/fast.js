...
function fastDataArrayToString(bot, fastData, showFastEndMessage = false, PREFIX = '?', commandUsed = 'fast') {
...
function multipleFastsToString(bot, message, fastArray, numberOfFasts, userTimezoneOffset, entriesToSkip = 0, toArray = false) {
...
async function getCurrentOrRecentFastEmbed(bot, userID, fastIsInProgress, userTimezoneOffset, PREFIX, commandUsed = 'fast') {
    ...
    if (fastIsInProgress === true) {
        fastDataToString += fastDataArrayToString(bot, fastData, true, PREFIX, commandUsed);
    }
    else {
        fastDataToString += fastDataArrayToString(bot, fastData, false, PREFIX, commandUsed);
    }
...
async function getFastPostEmbedArray(bot, message, PREFIX, fastData, forceSkip = false) {
    let spamDetails = {
        lastTimestamp: null,
        closeMessageCount: 0,
    };
    ...
    do {
        ...
        if (postIndex === 1) {
            ...
            if (attachmentArray.size > 0) {
                ...
                if (collectedMessage !== "0" && collectedMessage !== "post" && collectedMessage !== "2") {
                    postIndex--;
                }
        ...
        if (collectedMessage === "stop") return false;
            ...
            let confirmClearMessage = await fn.getUserConfirmation(bot, message, PREFIX, removeFastWarning, forceSkip, "Fast Post: Remove Attachment");
            ...
            let confirmClearMessage = await fn.getUserConfirmation(bot, message, PREFIX, clearMessageWarning, forceSkip, "Fast Post: Clear Current Message");
            if (confirmClearMessage === true) {
            ...
            let confirmClearAll = await fn.getUserConfirmation(bot, message, PREFIX, clearAllWarning, forceSkip, "Fast Post: Clear All");
            ...
            let confirmOverwrite = await fn.getUserConfirmation(bot, message, PREFIX, addDefaultMessagePrompt, true, "Fast Post: Add Default Fast Message");
            if (confirmOverwrite === true) {
                if (fastBreaker === null) {
                    collectedMessage = `=============\nBroke my **${fn.millisecondsToTimeString(fastDurationTimestamp)}** fast!\n=============`;
                }
                else collectedMessage = `=============\nBroke my **${fn.millisecondsToTimeString(fastDurationTimestamp)}** fast with **${fastBreaker}**!\n=============`;
                fastPost.push(collectedMessage);
                fastPostMessagePrompt = `${fastPostMessagePrompt}\n${collectedMessage}`;
            }
        }
            ...
            let confirmOverwrite = await fn.getUserConfirmation(bot, message, PREFIX, addFullFastPrompt, true, "Fast Post: Add Full Fast");
            if (confirmOverwrite === true) {
                collectedMessage = `=============\n${fastDataArrayToString(bot, fastData)}\n=============`;
                fastPost.push(collectedMessage);
                fastPostMessagePrompt = `${fastPostMessagePrompt}\n${collectedMessage}`;
            ...
            if (fastPost.length) {
                ...
                if (fastPost.length === 1) {
                    ...
                    if (targetStringIndex >= 0) {
                        fastPostMessagePrompt = fastPostMessagePrompt.substring(0, targetStringIndex - 1);
                    }
                    else {
    ...
    while (true)
    fastPost = fn.getEmbedArray(fastPost, "Fast Post", true, false, fastEmbedColour);
    if (attachment !== null) {
    ...
    run: async function run(bot, message, commandUsed, args, PREFIX,
         ...
         */
            ...
            if (!seeCommands.includes(args[1]) && isNaN(args[1])) {
                message.channel.send(await getCurrentOrRecentFastEmbed(bot, authorID, fastIsInProgress, timezoneOffset, PREFIX, commandUsed));
            ...
            // it will be handled properly later based on the values passed in!
                ...
                if (seeType === "recent" || seeType === "current") {
                    return message.channel.send(await getCurrentOrRecentFastEmbed(bot, authorID, fastIsInProgress, timezoneOffset, PREFIX, commandUsed));
                }
                else if (seeType === "all") {
                ...
                if (pastFunctionality) {
                    ...
                    if (args[2] !== undefined) {
                        ...
                        let confirmSeeAll = await fn.getUserConfirmation(bot, message, PREFIX, confirmSeeMessage, forceSkip, `Fast: See ${args[2]} Fasts (${sortType})`);
                        ...
                        let confirmSeeAll = await fn.getUserConfirmation(bot, message, PREFIX, confirmSeeAllMessage, forceSkip, "Fast: See All Fasts");
                        if (!confirmSeeAll) return;
                    }
                    ...
                    const fastDataToStringArray = multipleFastsToString(bot, message, fastView, pastNumberOfEntriesIndex, timezoneOffset, 0, true);
                    ...
                    if (args[2 + shiftIndex]) {
                        if (args[2 + shiftIndex].toLowerCase() === "past") {
                            if (args[3 + shiftIndex] !== undefined) {
                                ...
                                const confirmSeePast = await fn.getUserConfirmation(bot, message, PREFIX, confirmSeePastMessage, forceSkip, `Fast: See ${args[1]} Fasts Past ${entriesToSkip} (${sortType})`);
                                ...
                                console.log({ fastView });
                                const fastDataToStringArray = multipleFastsToString(bot, message, fastView, pastNumberOfEntriesIndex, timezoneOffset, entriesToSkip, true);
                                await fn.sendPaginationEmbed(bot, message.channel.id, authorID, fn.getEmbedArray(
                ...
                const fastDataToString = `__**Fast ${pastNumberOfEntriesIndex}:**__\n` + fastDataArrayToString(bot, fastData, showFastEndMessage, PREFIX, commandUsed);
            ...
            // delete past #:
                ...
                if (deleteType === "past") {
                    ...
                    const fastDataToStringArray = multipleFastsToString(bot, message, fastCollection, numberArg, timezoneOffset, 0, true);
                    ...
                    const multipleDeleteConfirmation = await fn.getPaginatedUserConfirmation(bot, message, PREFIX, fastArray, multipleDeleteMessage, forceSkip, `Fast: Delete Past ${numberArg} Fasts (${sortType})`, 600000);
                ...
                if (deleteType === "many") {
                    ...
                    if (args[2 + shiftIndex]) {
                        if (args[2 + shiftIndex].toLowerCase() === "past") {
                            ...
                            const showFasts = multipleFastsToString(bot, message, fastCollection, pastNumberOfEntries, timezoneOffset, skipEntries, true);
                            ...
                            const multipleDeleteConfirmation = await fn.getPaginatedUserConfirmation(bot, message, PREFIX, fastArray, multipleDeleteMessage, forceSkip,
            ...
            if (isNaN(args[1])) {
                ...
                if (deleteType == "recent" || deleteType == "current") {
                    ...
                    let confirmDeleteAll = await fn.getUserConfirmation(bot, message, PREFIX, confirmDeleteAllMessage, forceSkip, "Fast: Delete All Fasts WARNING!");
            ...
            if (isNaN(args[1]) && args[1].toLowerCase() !== "recent" && args[1].toLowerCase() !== "current") {
                ...
                do {
                    ...
                    if (fastView.endTime === null) {
                        fastData = fastDocumentToDataArray(fastView, timezoneOffset, true);
                        showFast = fastDataArrayToString(bot, fastData, true, PREFIX, commandUsed);
                        isCurrent = true;
                    }
                    else {
                        fastData = fastDocumentToDataArray(fastView);
                        ...
                        if (!continueEdit) {
                            try {
                                ...
                                if (fastView) {
                                    ...
                                    continueEdit = await fn.getUserConfirmation(bot, message, PREFIX, continueEditMessage, forceSkip, `Fast: Continue Editing Fast ${pastNumberOfEntriesIndex}?`, 300000);
                                }
                                else {
            ...
            if (args[1] !== undefined) {
                ...
                const endTimestamp = fastData[1];
                let fastPost = await getFastPostEmbedArray(bot, message, PREFIX, fastData, forceSkip);
                ...
                fastPost.forEach(async (post, i) => {
                    fastPost[i] = post.setTitle(`${bot.channels.cache.get(postChannel).guild.member(authorID).displayName}'s Fast - ${endTimeToDate}`);
