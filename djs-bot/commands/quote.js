...
module.exports = {
  name: "quote",
  description:
    "Get a random quotation or setup a time to get a recurring quotation sent to your DMs!",
  ...
  run: async function run(
    ...
    quoteUsageMessage = fn.getMessageEmbed(
      quoteUsageMessage,
      "Quote: Help",
      quoteEmbedColour
    );
    if (quoteCommand === "help") return message.channel.send(quoteUsageMessage);
      ...
      quoteCommand === "st" ||
      quoteCommand === "s" ||
      quoteCommand === "settings" ||
      quoteCommand === "setting" ||
      quoteCommand === "configuration" ||
      quoteCommand === "config" ||
      quoteCommand === "c" ||
      quoteCommand === "edit" ||
      quoteCommand === "ed" ||
      ...
      do {
        let quoteSettings = await User.findOne(
          { discordID: authorID },
          { getQuote: 1, quoteInterval: 1, nextQuote: 1, tier: 1 }
        );
        if (!quoteSettings)
          return message.reply(
            `Try \`${PREFIX}settings\` to setup your settings!`
          );

        var quoteFields = quoteSettings.getQuote
          ? ["Get Quotes", "Next Quote", "Quote Interval"]
          : ["Get Quotes"];
        var continueEdit, fieldToEdit, fieldToEditIndex;
        const fieldToEditInstructions = "**Which field do you want to edit?**";
        const fieldToEditAdditionalMessage = quoteDocumentToString(
          quoteSettings,
          timezoneOffset
        );
            ...
            case 0:
              {
                ...
                if (typeof userEdit === "boolean") {
                  ...
                  if (userEdit) {
                    ...
                    if (!intervalArgs) return;
                      ...
                      if (!interval) {
                        fn.sendReplyThenDelete(
                          message,
                          `**INVALID TIME**... Try \`${PREFIX}date\` for help`,
                          60000
                        );
                        error = true;
                        continueEdit = true;
                  ...
                  if (!error) {
                    quoteSettings = await User.findOneAndUpdate(
                      { discordID: authorID },
                      {
                        $set: {
                          getQuote: userEdit,
      ...
      const embed = fn
        .getMessageEmbed(showQuote, "Quote", quoteEmbedColour)
