...
function getJournalTemplate(
  args,
  withMarkdown = true,
  journalEmbedColour = fn.journalEmbedColour
) {
  var journalView;
  if (args[1] !== undefined) {
    ...
    if (
      ...
      if (args[2] !== undefined) {
        ...
        if (
          ...
          journalView = fn.getMessageEmbed(
            fn.getWeeklyJournalGoalTemplate(false, withMarkdown),
            `Weekly Journal: Weekly Goals`,
            journalEmbedColour
          );
        } else if (
          weeklyType === "reflection" ||
          weeklyType === "r" ||
          weeklyType === "re" ||
          weeklyType === "ref" ||
          weeklyType === "refl" ||
          ...
          journalView = fn.getMessageEmbed(
            fn.getWeeklyJournalReflectionTemplate(false, withMarkdown),
            `Weekly Journal: Weekly Reflection`,
            journalEmbedColour
          );
        } else return false;
      } else
        journalView = fn.getMessageEmbed(
          fn.getWeeklyJournalFullTemplate(true, withMarkdown),
      ...
      if (args[2] !== undefined) {
        ...
        if (
          ...
          journalView = fn.getMessageEmbed(
            fn.getDailyJournalMorningTemplate(true, withMarkdown),
          ...
          journalView = fn.getMessageEmbed(
            fn.getDailyJournalNightTemplate(true, withMarkdown),
            `Daily Journal: Night`,
            journalEmbedColour
          );
        } else return false;
        ...
        journalView = fn.getMessageEmbed(
          fn.getDailyJournalFullTemplate(true, withMarkdown),
...
async function getGeneratedPromptAndAnswer(bot, message, PREFIX, prompts) {
  ...
  do {
    ...
    let entry = await fn.getMultilineEntry(
      ...
      true,
      journalEmbedColour,
      newPromptInstructions,
      newPromptKeywords,
      newPrompt ? "" : entry.array
    );
    if (!entry) return false;
      ...
      if (entry.message) {
        const confirmNewPrompt = await fn.getUserConfirmation(
          ...
          false,
          "Journal: New Prompt Confirmation"
        );
...
async function getOneJournalByObjectID(journalTargetID) {
  try {
    const entries = await Journal.findById(journalTargetID);
    console.log(entries);
    return entries;
  } catch (err) {
    console.log(err);
...
async function getMostRecentJournal(
  bot,
  userID,
  embedColour = journalEmbedColour
) {
  const recentJournalToString = `__**Journal ${await getRecentJournalIndex(
    userID
  )}:**__\n${journalDocumentToString(
    bot,
    await getOneJournalByRecency(userID, 0)
  )}`;
  const journalEmbed = fn.getMessageEmbed(
    recentJournalToString,
    `Journal See Recent Entry`,
    embedColour
  );
  return journalEmbed;
}
...
function journalDocumentToString(bot, journalDoc) {
  ...
  let entryString = `**Created At:** ${
    createdAt ? fn.timestampToDateString(createdAt) : ""
  }\n**Type: **`;
  switch (template) {
    case 1:
...
function multipleJournalsToString(
  bot,
  message,
  journalArray,
  numberOfJournals,
  ...
  for (let i = 0; i < numberOfJournals; i++) {
    if (journalArray[i] === undefined) {
      ...
      fn.sendErrorMessage(
        message,
        `**JOURNALS ${i + entriesToSkip + 1}**+ ONWARDS DO NOT EXIST...`
      );
      break;
    }
    const journalString = `__**Journal ${
      i + entriesToSkip + 1
...
async function getRecentJournalIndex(userID) {
  try {
    ...
    if (entries.length) {
      let targetID = await Journal.findOne({ userID }).sort({ _id: -1 });
...
module.exports = {
  ...
  run: async function run(
    ...
    let journalUsageMessage =
      `**USAGE**\n\`${PREFIX}${commandUsed} <ACTION>\`` +
      `\n\n\`<ACTION>\`: **start/s; end/e; see; edit; delete/d; post; template/t**\n\n*__ALIASES:__* **${
        this.name
      } - ${this.aliases.join("; ")}**`;
    journalUsageMessage = fn.getMessageEmbed(
      journalUsageMessage,
      "Journal: Help",
      journalEmbedColour
    );
    ...
    const journalInProgress = await Journal.findOne({
      template: 1,
      userID: authorID,
      "entry.amazing": undefined,
      "entry.betterDay": undefined,
    });
    const totalJournalNumber = await Journal.find({
      userID: authorID,
    }).countDocuments();
    console.log({ journalInProgress, totalJournalNumber });
    // Journal Commands
    if (journalCommand === "help")
      return message.channel.send(journalUsageMessage);
    else if (
      journalCommand === "start" ||
      journalCommand === "st" ||
      journalCommand === "s" ||
      journalCommand === "set" ||
        ...
        case "📜": {
          if (journalInProgress)
            return message.reply(
              `**You already have a journal entry in progress!** Try** \`${PREFIX}${commandUsed} end\` **to **complete** your journal entry`
            );
          ...
          const confirmEndReminder = await fn.getUserConfirmation(
            ...
            false,
            "Journal: End of Day - Completion Reminder",
            180000
          );
          ...
          let endTime = await fn.getDateAndTimeEntry(
            ...
            "Journal: End of Day - Reflection Time",
            true,
            journalEmbedColour
          );
          if (!endTime) return;
          ...
          message.reply(
            `Journal end reminder set for **${fn.millisecondsToTimeString(
        ...
        case "🗣":
          {
            ...
            if (promptType === "🖋") {
              ...
              journalDocument
                ...
                .then((result) => {
                  message.reply(
                    "**Your journal entry was successfully created!**"
                  );
                  console.log({ result });
                })
                .catch((err) => console.error(err));
              ...
              promptArray = prompts;
              const journalEntry = await getGeneratedPromptAndAnswer(
                bot,
                message,
                PREFIX,
                promptArray
      ...
      const finishedJournal = await Journal.findByIdAndUpdate(
        journalInProgress._id,
        { $set: { "entry.amazing": amazing, "entry.betterDay": betterDay } },
        { new: true }
      );
      console.log({ finishedJournal });
      if (finishedJournal) {
        console.log(
          `Completing ${authorUsername}'s (${authorID}) journal entry!`
        );
        ...
        return console.log(
          `There was an error completing ${authorUsername}'s (${authorID}) journal entry`
        );
      return;
    } else if (
      journalCommand === "delete" ||
      journalCommand === "remove" ||
      journalCommand === "del" ||
      journalCommand === "d" ||
      journalCommand === "rem" ||
      journalCommand === "r"
    ) {
      let journalDeleteUsageMessage = fn.getReadOrDeleteUsageMessage(
        PREFIX,
        commandUsed,
        journalCommand,
        true,
      ...
      journalDeleteUsageMessage = fn.getMessageEmbed(
        journalDeleteUsageMessage,
        "Journal: Delete Help",
        journalEmbedColour
      );
      ...
      if (journalType) {
        ...
        if (!totalJournalNumber) {
          return message.reply(
            `**NO JOURNALS...** Try \`${PREFIX}${commandUsed} start\` to **start** one!`
          );
        }
      ...
      if (args[2] !== undefined) {
        ...
        if (deleteType === "past") {
          ...
          if (indexByRecency)
            journalCollection = await fn.getEntriesByRecency(
              Journal,
              { userID: authorID },
              0,
              numberArg
            ...
            journalCollection = await getJournalByCreatedAt(
              authorID,
              0,
              numberArg
            );
          const journalStringArray = fn.getEmbedArray(
            multipleJournalsToString(
              bot,
              message,
              journalCollection,
              numberArg,
          ...
          const multipleDeleteConfirmation = await fn.getPaginatedUserConfirmation(
            ...
            forceSkip,
            `Journal: Delete Past ${numberArg} Entries (${sortType})`,
            600000
          );
        ...
        if (deleteType === "many") {
          ...
          if (!toDelete.length) {
            return fn.sendErrorMessage(
              message,
              "All of these **journals DO NOT exist**..."
            );
          }
          ...
          for (let i = 0; i < toDelete.length; i++) {
            var journalView;
            if (indexByRecency) {
              journalView = await getOneJournalByRecency(
                authorID,
                toDelete[i] - 1
              );
            } else {
              journalView = await getOneJournalByCreatedTime(
                authorID,
                toDelete[i] - 1
              );
            }
            journalTargetIDs.push(journalView._id);
            journalStringArray.push(
              `__**Journal ${toDelete[i]}:**__\n${journalDocumentToString(
                bot,
                journalView
              )}`
            );
          ...
          const sortType = indexByRecency ? "By Recency" : "By Date Created";
          journalStringArray = fn.getEmbedArray(
            journalStringArray,
            "",
            true,
            false,
          ...
          const confirmDeleteMany = await fn.getPaginatedUserConfirmation(
            ...
            forceSkip,
            `Journal: Delete Entries ${toDelete} (${sortType})`,
            600000
          );
          ...
          if (args[2 + shiftIndex]) {
            if (args[2 + shiftIndex].toLowerCase() === "past") {
              ...
              const multipleDeleteConfirmation = await fn.getPaginatedUserConfirmation(
                ...
                multipleDeleteMessage,
                forceSkip,
                `Journal: Multiple Delete Warning! (${sortType})`
              );
              console.log({ multipleDeleteConfirmation });
              if (!multipleDeleteConfirmation) return;
              console.log({ multipleDeleteConfirmation });
              const targetIDs = await journalCollection.map(
                (entry) => entry._id
              );
              console.log(
      ...
      if (isNaN(args[1])) {
        ...
        if (deleteType === "recent") {
          ...
          const journalEmbed = fn.getEmbedArray(
            `__**Journal ${journalIndex}:**__\n${journalDocumentToString(
              bot,
              journalView
            )}`,
          ...
          let confirmDeleteAll = await fn.getUserConfirmation(
            ...
            forceSkip,
            "Journal: Delete All Entries WARNING!"
          );
        ...
        if (!journalView) {
          return fn.sendErrorMessageAndUsage(
            ...
            `**JOURNAL ${pastNumberOfEntriesIndex} DOES NOT EXIST**...`
          );
        }
        const journalTargetID = journalView._id;
        const sortType = indexByRecency ? "By Recency" : "By Date Created";
      ...
      let journalSeeUsageMessage = fn.getReadOrDeleteUsageMessage(
        PREFIX,
        commandUsed,
        journalCommand,
        true,
      ...
      journalSeeUsageMessage = fn.getMessageEmbed(
        journalSeeUsageMessage,
        "Journal: See Help",
        journalEmbedColour
      );
      ...
      if (journalType) {
        ...
        if (!totalJournalNumber) {
          ...
          return message.reply(
            `You have **${totalJournalNumber} journal entries** on record.`
          );
        }
      } else return message.reply(journalActionHelpMessage);
      ...
      // it will be handled properly later based on the values passed in!
        ...
        if (seeType === "recent") {
          return message.channel.send(
            await getMostRecentJournal(bot, authorID, journalEmbedColour)
          );
        } else if (seeType === "all") {
        ...
        if (pastFunctionality) {
          ...
          if (args[2] !== undefined) {
            ...
            let confirmSeeAll = await fn.getUserConfirmation(
              ...
              confirmSeeMessage,
              forceSkip,
              `Journal: See ${args[2]} Entries (${sortType})`
            );
          ...
          var journalView;
          if (indexByRecency)
            journalView = await fn.getEntriesByRecency(
              Journal,
              { userID: authorID },
              0,
            ...
            journalView = await getJournalByCreatedAt(
              authorID,
              0,
              pastNumberOfEntriesIndex
            );
          console.log({ journalView, pastNumberOfEntriesIndex });
          const journalStringArray = multipleJournalsToString(
            bot,
            message,
            journalView,
            pastNumberOfEntriesIndex,
          ...
          await fn.sendPaginationEmbed(
            ...
            fn.getEmbedArray(
              ...
              true,
              `Journals ${fn.timestampToDateString(
            ...
            if (args[2 + shiftIndex].toLowerCase() === "past") {
              if (args[3 + shiftIndex] !== undefined) {
                ...
                const confirmSeePast = await fn.getUserConfirmation(
                  ...
                  forceSkip,
                  `Journal: See ${args[1]} Entries Past ${entriesToSkip} (${sortType})`
                );
        ...
        const journalEmbed = fn.getEmbedArray(
          ...
          true,
          `Journal ${fn.timestampToDateString(
            Date.now() + timezoneOffset * HOUR_IN_MS,
            false,
            false,
            true,
      ...
      journalCommand === "edit" ||
      journalCommand === "ed" ||
      journalCommand === "change" ||
      journalCommand === "c" ||
      journalCommand === "update" ||
      journalCommand === "upd" ||
      journalCommand === "ch"
    ) {
      ...
      if (journalType) {
        ...
        if (!totalJournalNumber) {
          return message.reply(
            `**NO JOURNALS**... try \`${PREFIX}${commandUsed} help\` to set one up!`
          );
        }
        if (isNaN(journalType) && journalType !== "recent") {
          ...
          var journalDocument;
          if (indexByRecency)
            journalDocument = await getOneJournalByRecency(
              authorID,
              pastNumberOfEntriesIndex - 1
            );
          else
            journalDocument = await getOneJournalByCreatedTime(
              authorID,
              pastNumberOfEntriesIndex - 1
            );
          if (!journalDocument) {
          ...
          switch (template) {
            case 1:
              journalFields = journalFields.concat([
                "Gratitudes",
                "Actions/Mindsets for Great Day",
                "Affirmations",
                "Amazing Things",
                "Retrospective Better Day",
              ]);
              break;
            case 2:
          ...
          do {
            ...
            if (!continueEdit) {
              try {
                console.log(
                  `Editing ${authorID}'s Journal ${pastNumberOfEntriesIndex} (${sortType})`
                );
                if (fieldToEditIndex === 0) {
                  journalDocument = await Journal.findOneAndUpdate(
                    { _id: journalTargetID },
                    { $set: { createdAt } },
                    { new: true }
                  );
                ...
                if (journalDocument) {
                  pastNumberOfEntriesIndex = indexByRecency
                    ? await fn.getEntryIndexByFunction(
                        ...
                        journalTargetID,
                        totalJournalNumber,
                        getOneJournalByRecency
                      )
                    : await fn.getEntryIndexByFunction(
                        authorID,
                        journalTargetID,
                        totalJournalNumber,
                        getOneJournalByCreatedTime
                      );
                  console.log({
                    journalDocument,
                    journalTargetID,
                  ...
                  continueEdit = await fn.getUserConfirmation(
                    ...
                    continueEditMessage,
                    forceSkip,
                    `Journal: Continue Editing Journal ${pastNumberOfEntriesIndex}?`,
                    300000
                  );
                } else {
      ...
      if (!totalJournalNumber)
        return message.reply(
          `**No journal entries...** Try \`${PREFIX}${commandUsed} start\` to **start** one!`
        );
      if (args[1] !== undefined) {
        let journalIndex = isNaN(args[1])
          ? args[1].toLowerCase()
          : parseInt(args[1]);
        let indexByRecency = false;
        if (journalIndex === "recent") {
          ...
          return message.reply(
            `**Please enter a number or \"recent\" after \`${PREFIX}${commandUsed} ${journalCommand} <# | recent>\`**`
          );
        journalIndex--;
        if (journalIndex < 0 || journalIndex >= totalJournalNumber) {
          return message.reply(
            `**Journal ${journalIndex + 1} does not exist**`
          );
        }
        var journal;
        if (indexByRecency)
          journal = await getOneJournalByRecency(authorID, journalIndex);
        ...
        const posts = fn.getEmbedArray(
          journalDocumentToString(bot, journal),
          `${
            member ? `${member.displayName}'s ` : ""
          }Journal Entry - ${fn.timestampToDateString(
            journal.createdAt,
            false,
            true,
            true
    ...
    // SHOWS WEEKLY JOURNAL TEMPLATES!
      ...
      journalCommand === "template" ||
      journalCommand === "templates" ||
      journalCommand === "temp" ||
      journalCommand === "t"
    ) {
      ...
      if (journalType === "help")
        return message.channel.send(templateUsageMessage);
